...
%                    NetMes_Bin{count,1}= effciency_node (str3,str2,prefix,filename,first,last,step,number);
%                    NetMes_Bin{count,2}='Node efficiency';
                   %save(str2,'NetMes_Bin');
...
a1 = dir(str11);
b1 = length(a1);
% pathname = uigetdir(cd, a1);
% filesmat = ls(strcat(pathname,'/*.mat'));
% files = cellstr(filesmat); % 得到文件路径
...
for i1=1:b1   %被试文件夹
    ...
    elseif strcmp(filename1,str2)==1 
        ...
        for i2=1:b2   %稀疏度文件
            ...
            if strcmp(filename2,str1)==1 
            elseif strcmp(filename2,str2)==1 
            else                                                        
...
if s1=='1'
    %bform = Bform(str1,str3,number);
    %getNetworkallshortpathproperty(str3,str2,prefix,filename,first,last,step);
    count = count + 1;             
...
if s3=='1'
    %degree= getNodedegreeproperty (str1,str2,prefix,filename,first,last,step);
    %degreesum( degree,degree,number )
     count = count + 1;             
